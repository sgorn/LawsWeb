    ...
    public partial class Recherche : System.Web.UI.Page
    {
        ...
        private void createResultsDisplayInfoAndPageNav(int pagenum, int startParam)
        {
            if (searchParams != null && !String.IsNullOrEmpty(this.searchParams.AutonomyID) && !String.IsNullOrEmpty(this.searchParams.HitNumber))
            {
                ...
                Label2.Text = "Vous n’avez pas trouvé ce que vous cherchiez? <br /><a href='http://recherche-search.gc.ca/rGs/s_r?st=s&s5bm3ts21rch=x&num=10&st1rt=0&langs=fra&cdn=canada&q=" + txtS3archA11.Text + "'>Effectuez une recherche dans tous les sites Web du gouvernement du Canada</a>";
                if (hitid > 0)
                {
                    pageLinks += "<a href='" + this.BuildQueryString(filterType.PreviousID, hitID, true) + "'>Résultat précédent</a>";
                    pageLinks += " | ";
                }
                pageLinks += "<a href='" + this.BuildQueryString(filterType.PageID, hitID, false) + "'>Retourner &agrave; la liste de résultats</a>";
            ...
            {  //multiple pages are needed
                ...
                if (prevpage > 0)
                {
                    pageLinks += "<a href='" + this.BuildQueryString(filterType.Page, prevpage, ! this.chkTitlesOnly.Checked)
                ...
                {
                    pageLinks += "<a href='" + this.BuildQueryString(filterType.Page, nextpage, ! this.chkTitlesOnly.Checked)
        ...
        public string GetHitLink(object dataItem)
        {
            ...
            //in hits only mode, with a keyword, append keyword to the hit link so that it will highlight on law page
            //if ((!this.rdoContext.Checked) && (!this.textIsEmpty))
            //{
            //    if (link.Contains("#"))
            //    {
            //        int i = link.IndexOf(".html") + 5;
            //        //quotes are stripped out because it breaks the WET searchhighlight widget
            //        link = link.Insert(i, "?term=" + HttpUtility.UrlEncode(txtS3archA11.Text.Replace("\"", "")));
            //    }
            //    else
            //    {
            //        link += "?term=" + HttpUtility.UrlEncode(txtS3archA11.Text);
            //    }
            //}
        ...
        public string writeTabParameter()
        {
            string txt = "";
            //if (this.currentResult != null && this.currentResult.HitsReturned > 0)
            //{
            //    txt = "tabs: { id: \"tab\", tabSelector: \"";
            //    for (int i = 1; i <= currentResult.HitsReturned; i++)
            //    {
            //        txt += "#tab-container" + i + ", ";
            //    }
